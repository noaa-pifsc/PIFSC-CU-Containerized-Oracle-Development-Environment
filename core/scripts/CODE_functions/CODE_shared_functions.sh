...
function code_shared_load_CODE_config()
{
	...
	if [[ -f "${active_project_config_path}/.active_project" ]]; then
		...
		source "${active_project_config_path}/.active_project"
...
function code_shared_define_project_inheritance()
{
	...
	if [[ -n "${project_name}" ]]; then
		...
		if  [[ -f "${projecs_path}${project_name}/config/project_parent_config.sh" ]]; then
